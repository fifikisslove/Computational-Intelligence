...
Part 1: With rbf non-linear features
...
def get_centers_and_sigma(n_center):
    """
    Create for a given number of centers (n_center) the numpy array containing the centers and provide a good sigma
    :param n_center:
    :return:
    """
    ...
    centers = np.linspace(-1, 1, n_center, endpoint=True)
    sigma = 2.0 / n_center
...
def design_matrix(x, centers, sigma):
    ...
    EX: for the data x = [0,1,2], the centers [0,1] and sigma = 1/sqrt(2)
    ...
    res = np.zeros((x.shape[0],centers.shape[0]))
    for i in range(0, x.shape[0],1):
        for j in range(0, centers.shape[0], 1):
            res[i][j] = np.exp((-(np.power((x[i] - centers[j]), 2)) / (2 * np.power(sigma,2))))
...
def compute_error(theta, n_centers, x, y):
    ...
    x = design_matrix(x,centers, sigma)
    err2 = np.power((np.dot(x, theta) - y),2)
    err = np.mean(err2)
...
def train_and_test(data, n_centers):
    """
    Train the model with the number of centers 'n_centers' and provide the MSE for the training, validation and testing sets

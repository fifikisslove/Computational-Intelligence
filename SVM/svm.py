...
from svm_plot import plot_svm_decision_boundary, plot_score_vs_degree, plot_score_vs_gamma, plot_mnist, plot_confusion_matrix
...
def ex_1_a(x, y):
    ...
    ###########
    a = svm.SVC(kernel="linear") #, C=0.025)
    a.fit(x, y)
    plot_svm_decision_boundary(a, x, y)
...
def ex_1_b(x, y):
    ...
    ###########
    ## TODO:
    ## And a point (4,0) with label 1 to the data set and then
    ## train an SVM with a linear kernel
    ## and plot the decision boundary and support vectors using 'plot_svm_decision_boundary' function
    ###########
    pass
...
def ex_3_a(x_train,y_train,x_test,y_test):
    ...
    ## - Mind the chance level and add the score obtain with the linear kernel as optional argument of this function
    ###########
...
def ex_3_b(x_train, y_train, x_test, y_test):
    ...
    ## Plot the 10 first miss-classified digits using plot_mnist.
    ###########

    labels = range(1,6)

    sel_error = np.array([0]) # Replace be as selecting the selecting the miss-classified examples only
